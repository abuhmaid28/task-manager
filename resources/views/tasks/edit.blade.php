...
@section('content')
    <div class="max-w-lg mx-auto bg-white shadow-md rounded-lg p-8 mt-10">
        <h1 class="text-2xl font-semibold text-primary mb-6">Edit Task</h1>
        ...
        <form action="{{ route('tasks.update', $task->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description"
                    class="w-full p-3 border border-lightGray rounded focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent">{{ $task->description }}</textarea>
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4">
                <label for="status" class="inline-flex items-center">
                    <input type="checkbox" name="status" id="status" value="1"
                        class="form-checkbox h-5 w-5 text-primary" {{ $task->status ? 'checked' : '' }}>
                    <span class="ml-2 text-gray-700">Mark as Completed</span>
                </label>
            </div>
            <div class="flex justify-end">
                <button type="submit"
                    class="bg-primary text-white py-3 px-6 rounded-lg hover:bg-opacity-90 transition">Update Task</button>
            </div>
        </form>
    </div>
@endsection
